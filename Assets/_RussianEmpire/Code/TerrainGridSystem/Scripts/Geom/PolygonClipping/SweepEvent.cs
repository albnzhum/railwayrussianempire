...
namespace TGS.Geom
{
     ...
     */
        ...
        public SweepEvent(Point p, bool isLeft, PolygonType polygonType) : this(p, isLeft, polygonType, null,
            EdgeType.NORMAL)
        {
        }

        public SweepEvent(Point p, bool isLeft, PolygonType polygonType, SweepEvent otherSweepEvent) : this(p, isLeft,
            polygonType, otherSweepEvent, EdgeType.NORMAL)
        {
        }
        ...
        {
            bool equal = isLeft == e2.isLeft && polygonType == e2.polygonType && inOut == e2.inOut &&
                         edgeType == e2.edgeType && inside == e2.inside && Point.EqualsBoth(p, e2.p);
            if (!equal) return false;

            return otherSE.isLeft == e2.otherSE.isLeft && otherSE.polygonType == e2.otherSE.polygonType &&
                   otherSE.inOut == e2.otherSE.inOut && otherSE.edgeType == e2.otherSE.edgeType &&
                   otherSE.inside == e2.otherSE.inside && Point.EqualsBoth(otherSE.p, e2.otherSE.p);
        }
    }
}