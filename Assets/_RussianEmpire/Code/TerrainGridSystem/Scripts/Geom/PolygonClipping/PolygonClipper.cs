...
//		}
        ...
        void ProcessSegment(Segment segment, PolygonType polygonType)
        {
            ...
            {
                // the segment is vertical. The bottom endpoint is processed before the top endpoint 
                e2.isLeft = false;
            }
            else
            {
        ...
        Polygon ComputeInternal(PolygonOp operation)
        {
            ...
            while (!eventQueue.isEmpty)
            {
                ...
                if ((operation == PolygonOp.INTERSECTION && e.p.x > MINMAX_X) || (operation == PolygonOp.DIFFERENCE &&
                        e.p.x > subjectBB.right + Point.PRECISION))
        ...
        void PossibleIntersection(SweepEvent e1, SweepEvent e2)
        {
            ...
            if (sortedEvents.Count == 2)
            {
                ...
                e2.edgeType = e2.otherSE.edgeType =
                    ((e1.inOut == e2.inOut) ? EdgeType.SAME_TRANSITION : EdgeType.DIFFERENT_TRANSITION);
                return;
            }
            ...
            sortedEvents[1].edgeType = sortedEvents[1].otherSE.edgeType = EdgeType.NON_CONTRIBUTING;
            DivideSegment(sortedEvents[0], sortedEvents[1].p);
            sortedEvents[3].otherSE.edgeType =
                (e1.inOut == e2.inOut) ? EdgeType.SAME_TRANSITION : EdgeType.DIFFERENT_TRANSITION;
